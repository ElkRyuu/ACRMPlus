	...
	<form action="${ctp}/drain/list" method="post"> 
		...
		<c:if test="${!empty requestScope.page.content }">
			<table class="data_list_table" border="0" cellPadding="3" cellSpacing="0">
				...
				<c:forEach items="${requestScope.page.content }" var="drain">
					<tr>
						...
						<td class="list_data_text">${drain.status }</td>
						<td class="list_data_op">
							<img onclick="window.location.href='${ctp}/drain/confirm/edit?id=${drain.id }'" 
								title="确认流失" src="${ctp}/static/images/bt_confirm.gif" class="op_button" />
							<c:if test="${drain.status != '流失'}">
								<img onclick="window.location.href='${ctp}/drain/delay/edit?id=${drain.id }'" 
